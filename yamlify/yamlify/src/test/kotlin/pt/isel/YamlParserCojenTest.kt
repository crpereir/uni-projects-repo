...
class YamlParserCojenTest {
    ...
    fun `yaml to date`() {
        val yaml = """
          ...
          birth: 2004-05-26
        """.trimIndent()
        val cr = YamlParserCojen.yamlParser(StudentWithAnnotation::class).parseObject(yaml.reader())
    ...
    fun `yaml to time`() {
        val yaml = """
            time: 15-32-57
        """.trimIndent()
        val cr = YamlParserCojen.yamlParser(Time::class).parseObject(yaml.reader())
