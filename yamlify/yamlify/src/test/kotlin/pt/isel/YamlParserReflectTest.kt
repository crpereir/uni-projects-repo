...
class YamlParserReflectTest {
    ...
    fun `yaml to date`() {
        val yaml = """
          ...
          birth: 2004-05-26
        """.trimIndent()
        val cr = YamlParserReflect.yamlParser(StudentWithAnnotation::class).parseObject(yaml.reader())
        assertEquals(LocalDate::class, cr.birth::class)
    }
    ...
    fun `yaml to time`() {
        val yaml = """
            time: 15-32-57
        """.trimIndent()
        val cr = YamlParserReflect.yamlParser(Time::class).parseObject(yaml.reader())
        assertEquals(15,cr.time.hour)
    }
